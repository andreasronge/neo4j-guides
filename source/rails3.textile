...
The Neo4j::Rails::Model and Neo4j::Rails::Relationship classes does create automatically create transaction if needed. If you need to write several operations in one operation use @Neo4j::Rails::Transaction@. You will also get better performance using a single transaction instead of several small transactions.
...
TIP: Of course you can also subclass the @Neo4j::Rails::Relationship@ class just like for @Neo4j::Rails::Model@ class to domain specific behavour (e.g. validations, callbacks, business logic etc.)


h4. Create with a has_n/has_one accessor method

The has_n and has_one class methods can generate some convinience method for creating and traversing relationships and nodes.
...
You can access nodes with same method that creates relationship: @outgoing@, @incoming@ and tha has_n and has_one accessor methods.
All these methods returns an object thats includes the Ruby Enumerable mixin (access to each, collect, find etc).
