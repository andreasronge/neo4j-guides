...
It is not required to declare an property in order to use an index on that property (as in Neo4j.rb version <= 0.4.6).
...
TIP: has_list is not available if you are using online backup or ha cluster, since it does not yet work with those.
