...
Rules can be used to group a set of nodes. Rules and functions is very fast for reads. 
The grouping of nodes take place when you update, insert, delete a node, relationship or property.
So, if you have not many writes but mostly read operations using rules and function might give you better performance compared to using lucene queries or traversals.

Example of rule groups: all nodes of a class, property x == y, relationship z == q.
...
When a node change rule group @young@ it will trigger incoming relationship @readers@ (defined by has_n :readers).
<ruby>
