...
TIP: Neo4j/Rails Neo4j::Model already includes one rule: <pre>rule(:all, functions => Count.new)</pre>. That
means when calling the count method on the Person.all.count it will not traverse and count all nodes, instead
it will read the count function value on the rule node, which is much faster.
